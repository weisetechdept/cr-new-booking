...
$rate_limit_file = "../logs/rate_limit_{$ip}.txt";
$current_time = time();
$request_limit = env('RATE_LIMIT_REQUESTS', 10); // 10 requests per minute
$time_window = env('RATE_LIMIT_WINDOW', 60); // 1 minute
...
$max_range = env('MAX_DATE_RANGE_DAYS', 365) * 24 * 60 * 60; // configurable max range
if (($todate_timestamp - $fmdate_timestamp) > $max_range) {
    http_response_code(400);
    echo json_encode(['error' => 'Date range too large. Maximum ' . env('MAX_DATE_RANGE_DAYS', 365) . ' days']);
    exit;
}
...
try {
    ...
    // Log error
    error_log("API Error: " . $e->getMessage() . " - User: {$_SESSION['username']} - IP: {$ip}", 3, '../logs/api_errors.log');
